...
const promotionsSeed = [
  { code: 'WELCOME10', discount: '10%', scope: 'All events', uses: '124 uses', active: true },
  { code: 'KHMERFEST', discount: '$5', scope: 'Khmer Cultural Concert', uses: '68 uses', active: true },
  { code: 'VIP2026', discount: '15%', scope: 'VIP tickets only', uses: '31 uses', active: true },
];
...
const StatCard: React.FC<{ label: string; value: string; detail: string; icon: React.ElementType; tone?: string }> = ({ label, value, detail, icon: Icon, tone = 'indigo' }) => (
  <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-xs">
    <div className="flex items-start justify-between gap-3">
      <div><p className="text-[11px] font-semibold text-slate-500">{label}</p><p className="mt-1.5 text-xl font-extrabold text-slate-900">{value}</p><p className="mt-1 text-[10px] text-slate-400">{detail}</p></div>
...
export const AdminOperationsView: React.FC<AdminOperationsViewProps> = ({ feature }) => {
  ...
  const approveRefund = (id: string) => { setRefunds((rows) => rows.map((row) => row.id === id ? { ...row, status: 'APPROVED' } : row)); notify('បានអនុម័តសំណើសងប្រាក់', 'Refund request approved'); };
  const markRefundPaid = (id: string) => { setRefunds((rows) => rows.map((row) => row.id === id ? { ...row, status: 'PAID' } : row)); notify('បានសម្គាល់ថាបានសងប្រាក់', 'Refund marked as paid'); };
  const notify = (km: string, en: string) => { setActionMessage(text(km, en)); window.setTimeout(() => setActionMessage(''), 2500); };
  const exportReport = (name: string) => {
    const csv = `Report,Value\n${name},Generated ${new Date().toISOString()}\nTotal revenue,32540\nCompleted orders,486\nTickets sold,1248`;
    const link = document.createElement('a');
    link.href = `data:text/csv;charset=utf-8,${encodeURIComponent(csv)}`;
    link.download = `${name.toLowerCase().replace(/ /g, '-')}-report.csv`;
    link.click();
    notify('បានទាញយករបាយការណ៍រួចរាល់', 'Report downloaded successfully');
  };
  const savePromotion = () => {
    if (!promotionCode.trim() || !promotionDiscount.trim()) return;
    setPromotions((rows) => [{ code: promotionCode, discount: `${promotionDiscount}%`, scope: 'All events', uses: '0 uses', active: true }, ...rows]);
    setPromotionSaved(true);
    notify('បានបង្កើតប្រូម៉ូសិនថ្មី', 'Promotion created successfully');
  };

  return (
    <div className="admin-operations space-y-4 pb-8">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1.5 text-xl font-extrabold tracking-tight text-slate-900">{isKhmer ? meta.km : meta.en}</h2>
        ...
        <div className="flex items-center gap-2"><span className="px-3 py-2 rounded-lg bg-emerald-50 border border-emerald-200 text-xs font-semibold text-emerald-700 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500" />{text('ទិន្នន័យបានធ្វើបច្ចុប្បន្នភាព', 'Data up to date')}</span><button onClick={() => notify('បានធ្វើបច្ចុប្បន្នភាពទិន្នន័យ', 'Data refreshed successfully')} className="p-2 rounded-lg border border-slate-200 bg-white text-slate-500 hover:bg-slate-50" title={text('ធ្វើបច្ចុប្បន្នភាព', 'Refresh')}><RefreshCw className="w-4 h-4" /></button></div>
      </div>

      {actionMessage && <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm font-semibold flex items-center gap-2"><CheckCircle2 className="w-4 h-4" />{actionMessage}</div>}
      ...
      {feature === 'promotions' && <div className="grid grid-cols-1 xl:grid-cols-3 gap-5"><div className="xl:col-span-2 bg-white border border-slate-200 rounded-2xl p-5"><div className="flex items-center justify-between"><div><h3 className="font-bold text-slate-900">{text('កូដបញ្ចុះតម្លៃសកម្ម', 'Active discount codes')}</h3><p className="text-xs text-slate-500 mt-1">{text('បង្កើត និងគ្រប់គ្រងប្រូម៉ូសិន', 'Create and manage promotions')}</p></div><button onClick={() => document.getElementById('quick-promotion-code')?.focus()} className="px-3 py-2 rounded-lg bg-indigo-600 text-white text-xs font-semibold flex items-center gap-2"><Plus className="w-3.5 h-3.5" />{text('បង្កើតកូដថ្មី', 'New code')}</button></div><div className="mt-5 space-y-3">{promotions.filter((promotion) => promotion.active).map((promotion) => <div key={promotion.code} className="border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><div className="flex items-center gap-2"><span className="font-mono font-bold text-indigo-700">{promotion.code}</span><span className="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 text-[10px] font-bold">ACTIVE</span></div><p className="text-xs text-slate-500 mt-1">{promotion.discount} off · {promotion.scope} · {promotion.uses}</p></div><button onClick={() => { setPromotions((rows) => rows.map((row) => row.code === promotion.code ? { ...row, active: false } : row)); notify('បានបិទកូដបញ្ចុះតម្លៃ', 'Discount code deactivated'); }} className="text-xs font-semibold text-slate-500 hover:text-rose-600">{text('បិទកូដ', 'Deactivate')}</button></div>)}</div></div><div className="bg-slate-900 rounded-2xl p-5 text-white"><h3 className="font-bold">{text('បង្កើតប្រូម៉ូសិនរហ័ស', 'Quick promotion')}</h3><div className="mt-4 space-y-3"><label className="block text-xs text-slate-300">{text('កូដ', 'Code')}<input id="quick-promotion-code" value={promotionCode} onChange={(e) => setPromotionCode(e.target.value.toUpperCase())} className="mt-1 w-full rounded-lg bg-slate-800 border-slate-700 px-3 py-2 text-sm text-white" /></label><label className="block text-xs text-slate-300">{text('បញ្ចុះតម្លៃ (%)', 'Discount (%)')}<input value={promotionDiscount} onChange={(e) => setPromotionDiscount(e.target.value)} className="mt-1 w-full rounded-lg bg-slate-800 border-slate-700 px-3 py-2 text-sm text-white" /></label><button onClick={savePromotion} className="w-full mt-2 rounded-lg bg-indigo-500 hover:bg-indigo-400 py-2.5 text-xs font-bold">{promotionSaved ? text('បានរក្សាទុក ✓', 'Saved ✓') : text('រក្សាទុកប្រូម៉ូសិន', 'Save promotion')}</button></div></div></div>}

      {feature === 'organizers' && <div className="space-y-4"><div className="grid grid-cols-1 sm:grid-cols-3 gap-4"><StatCard label={text('អ្នករៀបចំសកម្ម', 'Active organizers')} value="18" detail={text('អ្នករៀបចំបានផ្ទៀងផ្ទាត់', 'Verified organizers')} icon={UsersRound} tone="indigo" /><StatCard label={text('កម្មវិធីសរុប', 'Total events')} value="42" detail={text('ក្នុងប្រព័ន្ធ', 'On the platform')} icon={Landmark} tone="violet" /><StatCard label={text('កំពុងពិនិត្យ', 'Pending review')} value="3" detail={text('ត្រូវការអនុម័ត', 'Needs approval')} icon={Activity} tone="amber" /></div><div className="bg-white border border-slate-200 rounded-2xl overflow-hidden"><div className="p-4 border-b border-slate-100 flex items-center justify-between"><div><h3 className="font-bold">{text('បញ្ជីអ្នករៀបចំកម្មវិធី', 'Organizer directory')}</h3><p className="text-xs text-slate-500 mt-1">{text('គ្រប់គ្រងអ្នករៀបចំ និងកម្មវិធីរបស់ពួកគេ', 'Manage organizers and their events')}</p></div><button onClick={() => notify('បើកទម្រង់បន្ថែមអ្នករៀបចំ', 'Organizer form is ready for a new entry')} className="px-3 py-2 rounded-lg bg-indigo-600 text-white text-xs font-semibold flex items-center gap-2"><Plus className="w-3.5 h-3.5" />{text('បន្ថែមអ្នករៀបចំ', 'Add organizer')}</button></div><div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead className="bg-slate-50 text-slate-500 uppercase"><tr><th className="px-4 py-3">Organizer</th><th className="px-4 py-3">Contact</th><th className="px-4 py-3">Events</th><th className="px-4 py-3">Sales</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{organizers.map((organizer) => <tr key={organizer.name}><td className="px-4 py-3 font-bold">{organizer.name}</td><td className="px-4 py-3 text-slate-500">{organizer.contact}</td><td className="px-4 py-3">{organizer.events}</td><td className="px-4 py-3 font-semibold">{organizer.sales}</td><td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-[10px] font-bold ${organizer.status === 'VERIFIED' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{organizer.status}</span></td><td className="px-4 py-3 text-right"><button onClick={() => notify(text('កំពុងបើកព័ត៌មានលម្អិតរបស់ ', 'Opening details for ') + organizer.name, text('កំពុងបើកព័ត៌មានលម្អិតរបស់ ', 'Opening details for ') + organizer.name)} className="text-indigo-600 font-semibold">{text('មើលលម្អិត', 'View details')}</button></td></tr>)}</tbody></table></div></div></div>}
