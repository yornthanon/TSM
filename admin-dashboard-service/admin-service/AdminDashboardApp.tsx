...
import { Bell, Calendar, CreditCard, LayoutDashboard, LogOut, Menu, Network, ShoppingCart, Ticket, Users, X } from 'lucide-react';
...
const items = [
  ...
  { id: 'api-gateway', km: 'API Gateway', en: 'API Gateway', captionKm: 'Routes និង API tester', captionEn: 'Routes & API tester', icon: Network },
];
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-slate-950 text-white border-r border-slate-800 transition-transform lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="p-4 space-y-1"><div className="px-3 pb-2 text-[10px] uppercase tracking-widest text-slate-500 font-bold">{isKhmer ? 'ម៉ូឌុល Admin' : 'Admin modules'}</div>{items.map(({ id, km, en, captionKm, captionEn, icon: Icon }) => <button key={id} onClick={() => go(id)} className={`w-full text-left flex items-center gap-3 px-3 py-3 rounded-xl transition ${activeTab === id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-950/40' : 'text-slate-400 hover:bg-slate-900 hover:text-white'}`}><Icon className="w-5 h-5 shrink-0" /><span className="min-w-0"><span className="block text-sm font-semibold truncate">{isKhmer ? km : en}</span><span className={`block text-[10px] mt-0.5 ${activeTab === id ? 'text-indigo-100' : 'text-slate-600'}`}>{isKhmer ? captionKm : captionEn}</span></span></button>)}</nav>
