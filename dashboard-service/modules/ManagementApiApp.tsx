...
export const ManagementApiApp: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800">
      <div className="lg:hidden sticky top-0 z-30 bg-slate-950 text-white px-4 py-3 flex items-center justify-between"><button onClick={() => setMobileOpen(!mobileOpen)} className="p-2 rounded-lg bg-slate-800">{mobileOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}</button><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-lg bg-cyan-600 flex items-center justify-center font-black text-xs">TSM</div><span className="font-bold">Management API</span></div><Activity className="w-5 h-5 text-cyan-300" /></div>
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-slate-950 text-white border-r border-slate-800 transition-transform lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 border-b border-slate-800"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center font-black">API</div><div><div className="font-bold tracking-tight">Management API</div><div className="text-[11px] text-slate-400 font-mono">PLATFORM OPERATIONS</div></div></div><div className="mt-5 rounded-xl bg-cyan-500/10 border border-cyan-400/20 px-3 py-2.5"><div className="text-[10px] uppercase tracking-widest text-cyan-300 font-bold">Workspace</div><div className="text-sm font-semibold mt-1">Gateway & Diagnostics</div></div></div>
        <nav className="p-4 space-y-1"><div className="px-3 pb-2 text-[10px] uppercase tracking-widest text-slate-500 font-bold">API modules</div><button onClick={() => go('api-manager')} className={`w-full text-left flex items-center gap-3 px-3 py-3 rounded-xl ${activeTab === 'api-manager' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:bg-slate-900 hover:text-white'}`}><Server className="w-5 h-5" /><span><span className="block text-sm font-semibold">API & Services</span><span className="block text-[10px] mt-0.5 opacity-70">Health, routes & errors</span></span>{errorCount > 0 && <span className="ml-auto px-1.5 py-0.5 rounded bg-rose-500 text-white text-[10px] font-mono">{errorCount}</span>}</button><button onClick={() => go('notifications')} className={`w-full text-left flex items-center gap-3 px-3 py-3 rounded-xl ${activeTab === 'notifications' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:bg-slate-900 hover:text-white'}`}><Network className="w-5 h-5" /><span><span className="block text-sm font-semibold">Event Stream</span><span className="block text-[10px] mt-0.5 opacity-70">Kafka notifications</span></span></button></nav>
