              ...
              {filteredTickets.map((ticket) => (
                <tr key={ticket.id} className="hover:bg-slate-50/80 transition">
                  ...
                  <td className="px-4 py-3 text-right">
                    {ticket.ticketStatus === 'AVAILABLE' && (
                      <button
                        onClick={() => onProceedToOrder && onProceedToOrder(ticket)}
                        className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded text-[11px] font-semibold shadow-xs"
                      >
                        Book
                      </button>
                    )}
